...
- `~` `jobs/data_quality_pipeline` — update
  - `~` `tasks[task_key='aggregate_results'].depends_on`: [1 items] -> [3 items]
  ...
  - `~` `tasks[task_key='compute_statistics'].depends_on`: [1 items] -> [2 items]
  - `~` `tasks[task_key='generate_report'].depends_on`: [1 items] -> [2 items]
