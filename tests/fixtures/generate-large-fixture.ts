...
const BASE_PATH = "/Workspace/Users/databricks.squiggly592@passfwd.com/.bundle/dagshund-test-bundle/default";
...
const buildQualityJob = (name: string, action: string, jobId: number) => {
  const tasks = [
    ...
    { task_key: "validate_schema", depends_on: ["setup"], notebook_path: `/${name}/validate_schema` },
    { task_key: "check_nulls", depends_on: ["validate_schema"], notebook_path: `/${name}/check_nulls` },
    { task_key: "check_duplicates", depends_on: ["validate_schema"], notebook_path: `/${name}/check_duplicates` },
    { task_key: "check_referential", depends_on: ["validate_schema"], notebook_path: `/${name}/check_referential` },
    { task_key: "check_ranges", depends_on: ["validate_schema"], notebook_path: `/${name}/check_ranges` },
    { task_key: "aggregate", depends_on: ["check_nulls", "check_duplicates", "check_referential", "check_ranges"], notebook_path: `/${name}/aggregate` },
...
const buildMlJob = (name: string, action: string, jobId: number) => {
  const tasks = [
    ...
    { task_key: "train_model_a", depends_on: ["split_data"], notebook_path: `/${name}/train_model_a` },
    { task_key: "train_model_b", depends_on: ["split_data"], notebook_path: `/${name}/train_model_b` },
    { task_key: "train_model_c", depends_on: ["split_data"], notebook_path: `/${name}/train_model_c` },
    ...
    { task_key: "select_best", depends_on: ["evaluate_a", "evaluate_b", "evaluate_c"], notebook_path: `/${name}/select_best` },
    { task_key: "register_model", depends_on: ["select_best"], notebook_path: `/${name}/register_model` },
...
const buildOrchestratorJob = (
  ...
  const tasks = [
    ...
    ...targetJobs.map((target, i) => ({
      ...
      depends_on: i === 0 ? [{ task_key: "prepare" }] : [{ task_key: `trigger_${targetJobs[i - 1]!.name}` }],
  ...
  remote_state: {
    ...
    settings: {
      ...
      tasks: [
        ...
        { task_key: "transform", depends_on: [{ task_key: "ingest" }], notebook_task: { notebook_path: "/Workspace/legacy/transform" } },
        { task_key: "load", depends_on: [{ task_key: "transform" }], notebook_task: { notebook_path: "/Workspace/legacy/load" } },
      ],
    },
  },
};
...
plan["resources.schemas.analytics"] = {
  action: "update",
  new_state: { value: { catalog_name: "dagshund", name: "analytics", comment: "Production analytics" } },
...
plan["resources.schemas.staging"] = {
  action: "create",
  new_state: { value: { catalog_name: "dagshund", name: "staging", comment: "Staging environment" } },
};

plan["resources.volumes.raw_data"] = {
  action: "update",
  depends_on: [{ node: "resources.schemas.analytics", label: "${resources.schemas.analytics.name}" }],
  new_state: { value: { catalog_name: "dagshund", schema_name: "analytics", name: "raw_data", volume_type: "MANAGED" } },
  remote_state: { catalog_name: "dagshund", schema_name: "analytics", name: "raw_data", volume_type: "MANAGED" },
};
...
plan["resources.registered_models.anomaly_detector"] = {
  action: "delete",
  remote_state: { catalog_name: "dagshund", schema_name: "analytics", name: "anomaly_detector", full_name: "dagshund.analytics.anomaly_detector" },
};
