...
All repository development and maintenance commands MUST run inside the
repository's Nix development shell. This includes Git, `br`, `just`, language
tooling, Databricks CLI, GitLab CLI, GitHub CLI, shell utilities, and every
command in agent guidelines and skills.

- For one-off commands, use `nix develop --command <command> ...`.
- For pipelines, redirection, command substitution, activation, or multiple
  commands, use `nix develop --command bash -c '<commands>'`.
- A human may run `nix develop` once and then work interactively inside that
  shell.
- Never rely on tools inherited from the host environment. Verify required
  tools are declared in `flake.nix`.
- End-user Dagshund usage and the bootstrap `nix develop` command itself are
  outside this repository-development rule.
