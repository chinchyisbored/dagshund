...
INTERACTIVE_FIXTURES=(lakebase-autoscaling manual-drift)
...
Interactive fixtures skipped by --all:
  lakebase-autoscaling  Run with human supervision:
                        just regen lakebase-autoscaling
                        Lakebase may leave the external dagshund-reader endpoint
                        list-visible but not get/delete-addressable.
...
regen_one() (
  ...
  if [[ "$name" == "lakebase-autoscaling" ]]; then
    echo "==> [$name] Interactive fixture: run with human supervision." >&2
  fi
